...
import { STATUS, ENVIRONMENT } from '@core/common/constants';
...
export class HttpExceptionFilter implements ExceptionFilter {
  private readonly logger: LogService = new LogService();
  ...
  constructor() {
    ...
    if (status_code === 404) {
      message =
        exception.message ||
        `Wrong url '${request.url}'. This url doesn't exist!`;
    }
    ...
    response.status(status_code).json({
      ...
      message,
      stack:
        process.env.NODE_ENV === ENVIRONMENT.DEVELOPMENT ? stack : undefined,
    });
  }
}
