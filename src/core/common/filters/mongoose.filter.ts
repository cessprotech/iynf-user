...
import { STATUS, MONGOOSE, ENVIRONMENT } from '@core/common/constants';

import { E_RESPONSE, MessageService } from '@core/modules/message';
import { IMongooseError } from '@core/common/interfaces';
import {
  handleCastErrorDB,
  handleCastObjectId,
...
export class MongooseExceptionFilter implements ExceptionFilter {
  @Logger(MongooseExceptionFilter.name) private readonly logger: LogService;
    ...
    if (
      exception.stack.includes(MONGOOSE.CAST_ERROR) &&
      exception.kind !== MONGOOSE.OBJECT_ID
    ) {
